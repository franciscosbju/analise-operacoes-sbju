...
def process_rima(data):
    ...
    data['PAX'] = data.apply(map_pax, axis=1)
    ...
    def find_consecutive(df, operation_type):
        ...
        for i in range(len(df) - 2):
            diff_total = (df.loc[i + 2, 'CALCO_DATETIME'] - df.loc[i, 'CALCO_DATETIME']).total_seconds() / 60
            if diff_total <= 45:
                results.append({
                    'First DateTime': df.loc[i, 'CALCO_DATETIME'].strftime('%d/%m/%Y %H:%M'),
                    'First Flight': f"{df.loc[i, 'AERONAVE_OPERADOR']} {int(df.loc[i, 'VOO_NUMERO'])}",
                    'Second DateTime': df.loc[i + 1, 'CALCO_DATETIME'].strftime('%d/%m/%Y %H:%M'),
                    'Second Flight': f"{df.loc[i + 1, 'AERONAVE_OPERADOR']} {int(df.loc[i + 1, 'VOO_NUMERO'])}",
                    'Third DateTime': df.loc[i + 2, 'CALCO_DATETIME'].strftime('%d/%m/%Y %H:%M'),
                    'Third Flight': f"{df.loc[i + 2, 'AERONAVE_OPERADOR']} {int(df.loc[i + 2, 'VOO_NUMERO'])}",
                    'PAX': df.loc[i, 'PAX'] + df.loc[i + 1, 'PAX'] + df.loc[i + 2, 'PAX']
                })
        return pd.DataFrame(results)

    # Análises para pousos e decolagens
    pousos = find_consecutive(data, 'P')
    decolagens = find_consecutive(data, 'D')
    return pousos, decolagens

# Processar o arquivo CSV
if rima_file is not None:
    try:
        rima_data = pd.read_csv(rima_file, delimiter=';')
        pousos_result, decolagens_result = process_rima(rima_data)
        ...
        if not pousos_result.empty:
            total_pousos = len(pousos_result)
            total_above_484 = len(pousos_result[pousos_result['PAX'] >= 484])
            st.markdown(f"<div>Total de Operações Consecutivas (Três Pousos): {total_pousos}</div>", unsafe_allow_html=True)
            st.markdown(f"<div class='highlight-red'>Total de Operações Superior a 484 PAX: {total_above_484}</div>", unsafe_allow_html=True)

            # Estilizar PAX
            styled_pousos = pousos_result.style.applymap(lambda x: 'color: red;' if isinstance(x, int) and x >= 484 else '', subset=['PAX'])
        ...
        if not decolagens_result.empty:
            total_decolagens = len(decolagens_result)
            total_above_484 = len(decolagens_result[decolagens_result['PAX'] >= 484])
            st.markdown(f"<div>Total de Operações Consecutivas (Três Decolagens): {total_decolagens}</div>", unsafe_allow_html=True)
            st.markdown(f"<div class='highlight-red'>Total de Operações Superior a 484 PAX: {total_above_484}</div>", unsafe_allow_html=True)

            # Estilizar PAX
            styled_decolagens = decolagens_result.style.applymap(lambda x: 'color: red;' if isinstance(x, int) and x >= 484 else '', subset=['PAX'])
